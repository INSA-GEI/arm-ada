...
#define APPLICATION_FIRST_ADDR				0x800C000
#define BAD_APPLICATION_RETURN_CODE		0xDEAD

static volatile uint32_t AppStack __attribute__((section("APP_STACK"))); // Debut de la stack applicative
	...
	else 
	{
		    ...
		    (mh->magic_str[3] == 'A'))
		{
			...
			if (mh->abi_version <= ABI_VERSION)
			{
				...
				/* La stack appli va de 0x10000000 à 0x10000C00, la stack systeme de 0x10000C00 à 0x10001000, 
				   le reste (0x1000) est reservée pour les données (data et bss) du systeme */
				__set_MSP((uint32_t)&AppStack);
				
				/* bascule en mode thread + psp */
				//__set_CONTROL(0x3); 
				...
				{
					fprintf(&__stdout,"Program terminated with exit code 0x%08X\n",return_val);
...
LSM303DLHCAcc_FilterConfigTypeDef LSM303DLHCFilter_InitStructure;
	...
	SYSTEM_AddITtoVector(USART3_IRQn, (int*)USART3_IRQHandler);
  ...
  NVIC_SetPriority(SysTick_IRQn,15);
	...
	printf ("\r\rSystem %d.%d\r", BL_MAJOR_VERSION, BL_MINOR_VERSION);
	printf ("Init [");
	
	printf ("Usart Systick IT Led ");
	LED_Init();
	LED_Set(LED_ON);
...
static int Counter_100Hz=0;
	...
	/* Gestion des evenements timer envoy�s vers l'applicatif */
	TIMER_Handler();
}
