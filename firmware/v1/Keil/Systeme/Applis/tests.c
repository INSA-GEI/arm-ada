...
/*const char* Test_Synth[] __attribute__((section("WAVES_TABLES"))) = {
...
SYNTH_Note note;
	...
	GLCD_DrawString(10, 9, "Press B to play note");
	GLCD_DrawString(3, 5, "Volume : ");
	GLCD_DrawString(3, 10, "Note : ");
	...
	{
		volume = POT_GetValue(POT_LEFT); /* Volume */
		val = POT_GetValue(POT_RIGHT); /* Note */
		note = val /(12*8);
		...
		//sprintf(strbuf, "%s    ",Test_Synth[note]);
		sprintf(strbuf, "%s    ", "toto");
		GLCD_DrawString(3, 12, strbuf);

		GUI_ProgressBar(120,78,170,20,volume, 0xFF);
		GUI_ProgressBar(120,158,170,20,note, (12*8));	
	} while (KEYS_GetState(KEY_A) != KEY_PRESSED);
