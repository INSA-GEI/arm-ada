...
uint8_t value;
	...
	while (!LCD_GET_WAIT); /* Attente que le signal wait repasse Ã  1 (fin du dernier traitement) */
...
uint32_t i;
	...
	i=0;
	while (!LCD_GET_WAIT); /* Attente que le signal wait LCD repasse à 1 (fin du dernier traitement) */
	...
	while (i< size)
	{
		/* Positionne l'adresse coté SRAM */
