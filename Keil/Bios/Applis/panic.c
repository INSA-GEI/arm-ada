...
static volatile uint32_t R0,R1,R2,R3,R12,PC,LR,XPSR;
...
char str[42];
	...
	GLCD_DrawString(0, 3, "Press B to erase flash and reset");
	
	GLCD_DrawString(0, 14, "System halted !");
	GLCD_DrawString(0, 10, "Backtrace :");
	sprintf (str, "R0[%08X] R1[%08X] R2[%08X]", R0, R1, R2);
	GLCD_DrawString(0, 11, str);
	sprintf (str, "R3[%08X] R12[%08X] LR[%08X]", R3, R12, LR);
	GLCD_DrawString(0, 12, str);
	sprintf (str, "PC[%08X] XPSR[%08X]", PC, XPSR);
