...
legacyMode = True # By default, run in legacy mode except if -v2 option is set on command line
adaExecutableName = ""
...
class Window(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.socketWorker = SocketWorker("armada.sock")
        ...
        if adaExecutableName != "":
            self.openFromCommandLine(adaExecutableName)
    ...
    def openFromCommandLine(self, filename):
        if self.execThread is None:
            pass
        else:
            if self.execThread.is_alive():
...
if len(sys.argv) > 1:
    if sys.argv[1] == "-v2":
        print ("enabling v2 mode")
        legacyMode = False

        if len(sys.argv) > 2:
            adaExecutableName = sys.argv[2]
            print ("Ada executable to run: " + adaExecutableName)
    else:
        adaExecutableName = sys.argv[1]
        print ("Ada executable to run: " + adaExecutableName)
