...
------------------------------------------------------------------------------
--
-- Cet acteur fourni les routines et ressources necessaires � la saisie
...
package body Morse is
   ...
   begin 
      ...
      if GetButtonState(BoutonCourt) then
         ...
         Symbole:=FinLettre;
      end if; -- Si ce n'est aucun de ces 3 cas, c'est donc le bouton finmot, d�ja selectionn� dans Symbole
      ...
      while GetButtonState(BoutonCourt) 
        ...
        or GetButtonState(BoutonFinMot) loop
         null; -- Attente que tous les boutons soient relach�s
      end loop;
