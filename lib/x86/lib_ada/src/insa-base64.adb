...
package body Insa.Base64 is
   ...
   function Encode(Source  : in  Ada.Streams.Stream_Element_Array) return String is
      -- Target  : out String;
      --  Last    : out Natural) is
      ...
      Output_Quad_Count: constant Natural := (Source'Length + 2) / 3;
      Output_Byte_Count: constant Natural := 4 * Output_Quad_Count;
      Target_Length : constant Natural := Output_Byte_Count + 2 * (Output_Byte_Count / 76);
      Target  : String(1..Target_Length);
