...
HAL_StatusTypeDef HAL_VirtualDMA_Start_IT(DMA_HandleTypeDef *hdma, uint32_t SrcAddress, uint32_t DstAddress, uint32_t DataLength);
#if LV_USE_GPU
...
/* Buffers definition used for display and work memory. */

__IO uintpixel_t my_fb[TFT_HOR_RES * TFT_VER_RES] __attribute__ ((section (".framebuffer"))); /* Framebuffer for display, for 1 layer */
lv_color_t buf1_1[LV_HOR_RES_MAX * 68] __attribute__ ((section (".sdram")));                      /* A buffer for 10 rows*/
lv_color_t buf1_2[LV_HOR_RES_MAX * 68] __attribute__ ((section (".sdram")));                      /* A buffer for 10 rows*/
lv_disp_buf_t disp_buf_1;
...
static void ex_disp_flush(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t * color_p)
{
	...
	while (y_fill_act <= y2_fill) {
		...
		length *= 2; /* STM32 DMA uses 16-bit chunks so multiply by 2 for 32-bit color */
#endif
		dest=(uint16_t *)((uint32_t)my_fb + (uint32_t)(TFT_HOR_RES*TFT_VER_RES*2) -
						  ((uint32_t)(&my_fb[y_fill_act * TFT_HOR_RES + x1_flush])-(uint32_t)my_fb));
		src=(uint16_t *)buf_to_flush;
		dest--;
